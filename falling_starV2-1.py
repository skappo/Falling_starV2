...
DEFAULTS = {
    ...
    "detection": {
        ...
        "base_threshold": 15,          # Start with a lower base for clean images
        "noise_factor": 2.5            # Key tuning parameter                                           
    },
    "timelapse": {
        "stack_N": 20,
        "stack_align": True,
        ...
        "auto_stack_method": True,
        "auto_stack_brightness_threshold": 40,
        "auto_sum_darkness_threshold": 5,
...
class DetectionConfig(BaseModel):
    ...
    adaptive_threshold_enabled: bool = DEFAULTS["detection"]["adaptive_threshold_enabled"]
    debug_visualization: bool = DEFAULTS["detection"]["debug_visualization"] 
...
class Pipeline:
    def __init__(self, cfg, config_path=None):
        ...
        self.diagnostics_dir = os.path.join(base_path, "output/diagnostics")
        self.daylight_out_dir = os.path.join(self.diagnostics_dir, "sky_conditions")
#        self.daylight_out_dir = os.path.join(base_path, "output/daylight")                                                                                           
        ...
        self.last_queue_status = ("ok", "Starting...")  # **NEW**: Holds (css_class, message)
        self.initial_sky_check_complete = threading.Event()
...
#                        logging.critical("="*60)
    ...
    def stack_frames(self, frames, align=True, method="mean", min_features_for_aligment=20):
        ...
        # --- START: NEW TWO-WAY "SMART STACKING" LOGIC ---
		
        effective_method = method  # Start with the user's preferred method
        
        try:
            cfg = self.cfg["timelapse"]

            auto_bright_override = cfg.get("auto_stack_method", False)
            bright_threshold = cfg.get("auto_stack_brightness_threshold", 40)
            
            dark_threshold = cfg.get("auto_sum_darkness_threshold", 5)

            # Analyze the brightness of the first frame once
            first_frame_gray = cv2.cvtColor(frames[0], cv2.COLOR_BGR2GRAY)
            mean_brightness = np.mean(first_frame_gray)

            # Logic 1: If user wants 'sum' but scene is too BRIGHT, force 'mean'
            if auto_bright_override and method == "sum" and mean_brightness > bright_threshold:
                logging.warning(
                    f"Auto Stack: Scene brightness ({mean_brightness:.1f}) exceeds threshold ({bright_threshold}). "
                    f"Overriding 'sum' method with 'mean' to prevent overexposure."
                )
                self.log_health_event("WARNING", "AUTO_STACK_BRIGHT_OVERRIDE", f"Brightness {mean_brightness:.1f} > {bright_threshold}")
                effective_method = "mean"

            # Logic 2: If user wants 'mean' but scene is too DARK, force 'sum'
            elif auto_bright_override and method == "mean" and mean_brightness < dark_threshold:
                logging.info(
                    f"Auto Stack: Scene brightness ({mean_brightness:.1f}) is below threshold ({dark_threshold}). "
                    f"Overriding 'mean' method with 'sum' to enhance stars."
                )
                self.log_health_event("INFO", "AUTO_STACK_DARK_OVERRIDE", f"Brightness {mean_brightness:.1f} < {dark_threshold}")
                effective_method = "sum"

        except Exception as e:
            logging.error(f"Error in auto-stack logic: {e}. Falling back to preferred method.")
            effective_method = method # Failsafe
        ...
        if effective_method == "median":
            ...
            if total_mem_required > (available_mem * 0.5):
                ...
                effective_method = "mean" # Force fallback
        ...
        if effective_method in ["mean", "sum"]:
...
#                        criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 100, 1e-7)
            ...
            if effective_method == "mean":
                ...
                # --- START: NEW BRIGHTNESS SCALING LOGIC ---
                try:
                    scale_factor = self.cfg["timelapse"].get("sum_brightness_scale", 1.0)
                    if scale_factor < 1.0:
                        logging.info(f"Applying brightness scale factor of {scale_factor} to summed stack.")
                        stacked_float *= scale_factor
                except Exception as e:
                    logging.error(f"Could not apply brightness scaling: {e}")
                # --- END: NEW LOGIC ---
...
#            ref_gray = cv2.cvtColor(frames[0], cv2.COLOR_BGR2GRAY) if align else None
            ...
            logging.error("Unknown stack_method '{effective_method}'. This should not happen.")
            # Default to the most common and safest method
            return None, None

        # Final conversion applies to all paths
        final_image = stacked_float.clip(0, 255).astype(np.uint8)
        return final_image, effective_method
            ...
            try:
                ...
                if item is None:
                    ...
                    if buffer:
                        ...
                        stacked_image, final_method = self.stack_frames(frames_to_stack, align=align, method=method, min_features_for_aligment=aligment_features)
                        if stacked_image is not None and final_method is not None:
                            ...
                            self.save_timelapse_metadata(full_path, final_method, buffer)
...
#                buffer.append((ts, frame.copy()))
        ...
        while True:
            try:
                ...
                if stacked_image is not None:
                    ...
                    cv2.imwrite(full_path, stacked_image, [int(cv2.IMWRITE_PNG_COMPRESSION), 3])
                    self.save_event_stack_metadata(full_path, event_frames, final_method)
    ...
    # ----------------- metadata helper -----------------
    def get_metadata(self, n_frames=1, mean_brightness=None, effective_threshold=None):
        g = self.cfg["general"]
#        exposure_us = self.cfg["capture"].get("exposure_us", 0)
    ...
    # -----------------
    def _save_generic_event_metadata(self, output_path, event_frames, stack_method=None):
        ...
        meta = self.get_metadata(n_frames=n_frames, mean_brightness=mean_brightness, effective_threshold=avg_threshold)
        ...
        if stack_method:
            ...
            if stack_method == "sum":
                meta["sum_brightness_scale"] = self.cfg["timelapse"].get("sum_brightness_scale", 1.0)
    ...
    def save_event_stack_metadata(self, img_path, event_frames, method_used):
        self._save_generic_event_metadata(img_path, event_frames, stack_method=method_used)

    # -----------------
    def save_timelapse_metadata(self, img_path, method_used, frames_buffer):
        ...
        while self.running.is_set():
            ...
            if self.producer_thread_active():
                ...
                if not self.daylight_mode_active.is_set():
                    if self.cfg["capture"].get("auto_exposure_tuning", False):
                ...
                # **NEW**: Signal that the initial check is complete.
                if not self.initial_sky_check_complete.is_set():
                    logging.info("Sky Monitor: Initial check complete. Releasing detection thread.")
                    self.initial_sky_check_complete.set()
    ...
    def capture_daylight_snapshot(self) -> Tuple[SkyConditionStatus, dict]:
        ...
        with self.camera_lock:        
            try:              
                ...
                # CRITICAL: Always restore night capture settings
                self.camera.picam2.set_controls({
                    "ExposureTime": self.current_exposure_us, # Use the potentially auto-tuned value - night_cfg.get("exposure_us"),
                    "AnalogueGain": self.current_gain,       # Use the potentially auto-tuned value - night_cfg.get("gain"),
                    "AwbEnable": False,
                    "AeEnable": False
                })
        ...
        with self.camera_lock:
            try:
                ...
                if "Lux" not in metadata:
                    logging.warning("Could not find 'Lux' value in camera metadata. Cannot determine operating mode.")
                    self.camera.picam2.set_controls({
                        "ExposureTime": self.current_exposure_us, "AnalogueGain": self.current_gain,
                        "AeEnable": False, "AwbEnable": False,
                        "ColourGains": [night_cfg.get("red_gain", 2.0), night_cfg.get("blue_gain", 1.5)]
                    })
                    return False
                ...
                # As a fallback, try to restore night settings to prevent being stuck in a bad state
                try:
                    self.camera.picam2.set_controls({
                        "ExposureTime": self.current_exposure_us, "AnalogueGain": self.current_gain,
                        "AeEnable": False, "AwbEnable": False,
        ...
        while self.running.is_set():
            ...
            if is_active_schedule and not is_capturing:
                if self.in_maintenance_mode.is_set():
                    ...
                    self.initial_sky_check_complete.clear()
                ...
                # try:
                    ...
                    # self.watchdog_pause.set()
                    # self.update_worker_list(self.threads_to_stop)
    ...
    def start_producer_thread(self):
        ...
        logging.info("Data production pipeline (CaptureThread and DetectionThread) started successfully.")
   
    # -----------------
    def stop_producer_thread(self):
        """
        Atomically and safely stops all data producer threads (e.g., Capture, Detection).
        This function is now fully responsible for managing the watchdog monitoring list
        to prevent race conditions during a normal shutdown.
        """
        # 1. Identify which producer threads are currently running and need to be stopped.
        ...
        # --- This is the critical, fast, and safe sequence ---
        try:
            logging.warning("Pausing watchdog for producer thread shutdown procedure.")
            self.watchdog_pause.set()
            
            # Immediately update the watchdog's list using the correct thread objects.
            self.update_worker_list(threads_to_stop)
        finally:
            # Always resume the watchdog immediately.
            logging.info("Resuming watchdog. Producer threads will now be joined.")
            self.watchdog_pause.clear()
        # --- The pause is now over. The long part can begin safely. ---

        # 3. Signal threads to begin their shutdown.
        ...
        # **NEW**: Reset the background model to ensure the next session starts fresh.
        logging.info("Resetting background model for next session.")
        self.background = None

        # 4. Wait for threads to finish. This can take time.
        logging.info("Waiting for producer threads to terminate...")
...
#        fps = self.cfg["capture"].get("fps", 2)
        ...
        while self.running.is_set():
            ...
            if not self.initial_sky_check_complete.is_set():
                try:
                    # Consume and discard frames from the queue to prevent it from filling up.
...
#            thr = self.cfg["detection"].get("threshold", 25)
            ...
            if self.daylight_mode_active.is_set() or self.weather_hold_active.is_set():
                effective_threshold = "N/A (Paused)"
                with self.status_lock: self.last_effective_threshold = effective_threshold
				# Still pass frame to timelapse with the status string
                try: self.timelapse_q.put_nowait((ts, frame, effective_threshold))
                except queue.Full: logging.warning("Timelapse queue is full.")
                continue               
            ...
            if cfg.get("adaptive_threshold_enabled", False):
                # Calculate the standard deviation of the difference image. This is a proxy for noise.
                # We use a small, central ROI to avoid bias from bright stars or vignetting at the edges.
                h, w = diff.shape
                roi = diff[h//4 : h*3//4, w//4 : w*3//4]
                _, std_dev = cv2.meanStdDev(roi)
                
                base_thr = cfg.get("base_threshold", 15)
                noise_factor = cfg.get("noise_factor", 2.5)
                
                # The core formula
                effective_threshold = int(base_thr + (std_dev[0][0] * noise_factor))
                
                # Clamp the value to a safe range (0-255)
                effective_threshold = max(0, min(255, effective_threshold))
                              
            else:
                # Fallback to the static threshold if the feature is disabled
                effective_threshold = cfg.get("static_threshold", 25)
                
            with self.status_lock:
                self.last_effective_threshold = effective_threshold

            try:
                self.timelapse_q.put_nowait((ts, frame, effective_threshold))
            except queue.Full:
                logging.warning("Timelapse queue is full, dropping frame.") 

            # --- Use the new effective_threshold in the OpenCV call ---
            _, th = cv2.threshold(diff, effective_threshold, 255, cv2.THRESH_BINARY)
            ...
            # ----------           
            if self.cfg["detection"].get("debug_visualization", False):
                # --- DEBUG VISUALIZATION BLOCK ---

                # Draw contours for visualization
                debug_frame = frame.copy()
                cv2.drawContours(debug_frame, cnts, -1, (0, 0, 255), 1)

                # Compute contour stats
                areas = [cv2.contourArea(c) for c in cnts]
                max_area = max(areas) if areas else 0

                logging.debug(f"[DEBUG] Threshold={effective_threshold}, Contours={len(cnts)}, MaxArea={max_area:.1f}")

                # Show intermediate images (for debugging only — do NOT use in production)
                # Resize for display so windows fit on screen
                h, w = gray.shape
                scale = 0.5

                # Resize first
                display_original = cv2.resize(frame, (int(w*scale), int(h*scale)))
                display_diff = cv2.resize(cv2.cvtColor(diff, cv2.COLOR_GRAY2BGR), (int(w*scale), int(h*scale)))
                display_th = cv2.resize(cv2.cvtColor(th, cv2.COLOR_GRAY2BGR), (int(w*scale), int(h*scale)))
                display_debug = cv2.resize(debug_frame, (int(w*scale), int(h*scale)))

                # --- Add labels BEFORE combining ---
                cv2.putText(display_original, "Original", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
                cv2.putText(display_diff, "Diff", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
                cv2.putText(display_th, "Threshold", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
                cv2.putText(display_debug, "Contours", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)

                # --- Combine in a 2x2 grid ---
                top_row = np.hstack((display_original, display_diff))
                bottom_row = np.hstack((display_th, display_debug))
                combined = np.vstack((top_row, bottom_row))

                cv2.imshow("Detection Debug (orig | diff | thresh | contours)", combined)

                # key = cv2.waitKey(1) & 0xFF
                # if key == ord('q'):
                    # logging.info("User requested quit from debug window.")
                    # self.running.clear()
                    # break
...
#            logging.info(f"ecc {event_cooldown_counter} su {cooldown_frames} - cef {current_event_frame_count} su {max_event_frames}")
            if self.event_in_progress.is_set() and (event_cooldown_counter > cooldown_frames or current_event_frame_count >= max_event_frames):
                ...
                try:
                    ...
                    if self.cfg.get("event_log", {}).get("enabled", True):
                        ...
                        log_summary = {
                            "timestamp_utc": end_dt.strftime("%Y-%m-%dT%H:%M:%SZ"), "start_time_utc": 
                            start_dt.strftime("%Y-%m-%dT%H:%M:%SZ"),
                            "duration_sec": (end_dt - start_dt).total_seconds(),
                            "num_frames_video": len(current_event_video_frames), "num_frames_video": 
                            len(current_event_stack_frames)
                        }
    ...
    def event_writer_loop(self):
        ...
        while True:
            ...
            try:
                for i, (ts, frame, _) in enumerate(frames):
                    fname = os.path.join(tmp_dir, f"f_{i:05d}.jpg")
                    cv2.imwrite(fname, frame, [int(cv2.IMWRITE_JPEG_QUALITY), self.cfg["events"]["jpeg_quality"]])

                video_path = os.path.join(out_dir, base_name + ".mp4")
                ff_cmd = [
                    "ffmpeg", "-y", "-framerate", str(self.cfg["events"]["video_fps"]),
                    "-i", os.path.join(tmp_dir, "f_%05d.jpg"),
                    "-c:v", self.cfg["events"]["ffmpeg_encoder"], 
                    "-b:v", self.cfg["events"]["ffmpeg_bitrate"], 
...
#            self.start_producer_thread()
        ...
        def generate_form_fields(data, prefix=''):
            html_out = ""
            EDITABLE_BOOLEANS = ["auto_exposure_tuning", "adaptive_threshold_enabled"]
            ...
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    ...
                    if isinstance(value, bool):
                        is_editable = key in EDITABLE_BOOLEANS
        ...
        def settings():
            ...
            <body>
              ...
              <div class="main">
                ...
                {save_success_banner}
                <div class="card">
                  <p style="color: #ccc;">This form shows the complete, active configuration.</p>
                  <p style="color: #e5c07b;"><b>Note:</b> Editable fields are highlighted in yellow.</p>
                  ...
                  <form action="/api/restart_service{token_param}" method="post" onsubmit="return confirm('Are you sure you want to restart the entire pipeline service? This will apply all saved configuration changes and interrupt any running captures.');">
                     <p style="color:#ccc;">If you have made significant changes (e.g., to file paths or thread settings), a full restart is recommended to apply them.</p>
                     <button type="submit" class="btn-danger">Restart Pipeline Service</button>
                  </form>
                </div>
              </div>
            </body></html>
...
#            import html
            ...
            with self.status_lock:
                status = { 
                    ...
                    "last_queue_status": self.last_queue_status # **NEW**: Get queue status
                }
#                calib_error = self.last_calibration_error
            with self.maintenance_timeout_lock:
...
#                            settings_html_rows += f'<tr><td>{param}</td><td><input type="text" name="{section}_{param}" value="{value}" size="10"> {note}</td></tr>'
            ...
            # **NEW**: Generate the queue status row conditionally
            queue_status_html = ""
            if self.cfg["queue_monitor"].get("enabled", False):
                q_class, q_message = status["last_queue_status"]
                # Use the 'title' attribute to show the full string on hover
                queue_status_html = f'<tr><td>Queue Status</td><td class="{q_class}" title="{html.escape(q_message)}">{html.escape(q_message)}</td></tr>'
...
#                    percent_full = 0
                ...
                logging.log(log_level, f"Queue Status | {status_string}")

                # **NEW**: Update the shared status variable for the dashboard
                status_class = "warn" if is_congested else "ok"
                with self.status_lock:
                    self.last_queue_status = (status_class, status_string)
        ...
        logging.info("Queue monitor has exited.")
    ...
    def perform_auto_exposure(self):
        """
        An intelligent auto-exposure algorithm designed for astrophotography.
        It adjusts gain and exposure time to meet a target background brightness
        while respecting constraints for meteor detection.
        """
        # This feature should only run in Night Mode
        if self.daylight_mode_active.is_set() or not self.running.is_set():
            return

        logging.info("Performing automatic exposure tuning check...")
        
        # Grab the configuration sandbox
        cfg = self.cfg["capture"]
        target = cfg.get("target_sky_brightness", 15)
        tolerance = 2  # A small dead-zone to prevent constant adjustments
        
        # Get a fresh frame using current settings
        with self.camera_lock:
            frame = self.camera.read()
        
        if frame is None:
            logging.warning("Auto-exposure: Failed to capture frame for analysis.")
            return

        # Analyze the image brightness using the median, which is robust against bright stars
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        current_brightness = np.median(gray)

        logging.info(f"Auto-exposure: Current median brightness is {current_brightness:.1f} (Target: {target})")

        new_exposure = self.current_exposure_us
        new_gain = self.current_gain
        change_made = False

        # --- The Core Tuning Logic ---
        if current_brightness < target - tolerance:
            # --- SCENE IS TOO DARK: Increase sensitivity ---
            # Priority 1: Increase exposure first, up to the meteor-safe maximum
            if self.current_exposure_us < cfg["max_exposure_us"]:
                new_exposure = min(self.current_exposure_us + 50000, cfg["max_exposure_us"]) # Increase by 50ms
                change_made = True
            # Priority 2: If exposure is maxed out, increase gain
            elif self.current_gain < cfg["max_gain"]:
                new_gain = min(self.current_gain + 0.5, cfg["max_gain"])
                change_made = True

        elif current_brightness > target + tolerance:
            # --- SCENE IS TOO BRIGHT: Decrease sensitivity ---
            # Priority 1: Decrease gain first, as it's the "noisier" parameter
            if self.current_gain > cfg["min_gain"]:
                new_gain = max(self.current_gain - 0.5, cfg["min_gain"])
                change_made = True
            # Priority 2: If gain is at minimum, decrease exposure
            elif self.current_exposure_us > cfg["min_exposure_us"]:
                new_exposure = max(self.current_exposure_us - 50000, cfg["min_exposure_us"]) # Decrease by 50ms
                change_made = True

        # If a change was calculated, apply it to the camera
        if change_made:
            logging.warning(f"Auto-exposure: Adjusting settings -> "
                          f"Exposure: {self.current_exposure_us} -> {new_exposure} | "
                          f"Gain: {self.current_gain:.1f} -> {new_gain:.1f}")

            with self.camera_lock:
                self.camera.picam2.set_controls({"ExposureTime": new_exposure, "AnalogueGain": new_gain})
            
            # Update our state variables
            self.current_exposure_us = new_exposure
            self.current_gain = new_gain
        else:
            logging.info("Auto-exposure: Current settings are optimal. No changes needed.")                       

    @staticmethod
    def create_master_dark(config_path, num_frames=50, out_path="master_dark.png"):
        """
        ...
        cfg = Pipeline.load_and_validate_config(config_path)
        ...
        try:
            ...
            master_dark = master_dark_float.clip(0, 255).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    if args.dark_frame:
        try:
            ...
            print(f"ERROR could not create the dark frame: {e}", file=sys.stderr)
        sys.exit(0)
